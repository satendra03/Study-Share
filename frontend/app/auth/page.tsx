...
export default function AuthPage() {
  const { signInWithGoogle, signInWithEmail, firebaseUser, loading } = useAuthStore();
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorStatus(err?.message || "Invalid email or password");
    }
  };
      ...
      <div className="relative z-10 w-full max-w-lg">
        ...
        <div className="bg-[#111116]/80 p-8 rounded-3xl border border-white/10 shadow-[0_0_40px_-15px_rgba(92,85,249,0.2)] backdrop-blur-xl">
          <h1 className="text-2xl md:text-3xl font-medium tracking-tight mb-2 text-center text-white">
            {useEmail ? "Teacher Login" : "Sign in to StudyShare"}
          </h1>
          <p className="text-gray-400 text-sm text-center mb-6">
            {useEmail ? "Access your account with email and password." : "Access study materials instantly safely & securely."}
          </p>
          ...
          {useEmail ? (
            <form onSubmit={handleEmailLogin} className="space-y-4">
              {errorStatus && (
                <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-xs text-center flex items-center gap-2 justify-center">
                  <AlertCircle className="w-4 h-4" /> {errorStatus}
                </div>
              )}
              <div className="space-y-1.5">
                <label className="text-xs text-gray-500 font-medium uppercase tracking-wider ml-1">Email Address</label>
                <input
                  type="email"
                  required
                  value={form.email}
                  onChange={e => setForm(prev => ({ ...prev, email: e.target.value }))}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500/50 transition-all"
                  placeholder="teacher@college.com"
                />
              </div>
              <div className="space-y-1.5 pb-2">
                <label className="text-xs text-gray-500 font-medium uppercase tracking-wider ml-1">Password</label>
                <input
                  type="password"
                  required
                  value={form.password}
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full flex items-center justify-center bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-4 rounded-full font-semibold transition-all active:scale-[0.98] disabled:opacity-70"
              >
          ...
          <div className="mt-8 pt-6 border-t border-white/5 text-center">
            <button
              ...
              className="text-xs cursor-pointer text-gray-500 hover:text-indigo-400 font-medium transition-colors uppercase tracking-wider"
            >
              {useEmail ? "← Back to Google Sign-in" : "Sign in as Teacher with Password"}
            </button>
          </div>
