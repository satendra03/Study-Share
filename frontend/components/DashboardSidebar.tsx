            ...
            >
              <span
                className={`flex items-center justify-center w-9 h-9 rounded-lg shrink-0 transition-colors ${
                  isActive ? "bg-[#5C55F9]/15 text-[#b4afff]" : "bg-white/4 text-gray-500 group-hover/nav:text-gray-300"
                }`}
              >
      ...
      <div className="relative px-3 pb-6 space-y-5 mt-auto">
        ...
        <div className="rounded-2xl border border-white/8 bg-white/3 p-3.5 backdrop-blur-sm">
          <div className="flex items-center gap-3 mb-3">
            <Avatar className="w-11 h-11 border border-white/15 rounded-xl shadow-inner ring-1 ring-[#5C55F9]/10">
              <AvatarImage src={firebaseUser?.photoURL || ""} className="rounded-xl object-cover" />
              <AvatarFallback className="rounded-xl text-white font-semibold bg-linear-to-br from-[#5C55F9] to-[#3730a3] text-sm">
          ...
          <div className="flex flex-col gap-0.5 border-t border-white/6 pt-3">
            ...
            <button
              type="button"
              onClick={() => void logout()}
              className="flex items-center gap-3 w-full px-2 py-2 text-sm text-gray-400 hover:text-red-400 transition-colors text-left rounded-lg hover:bg-red-950/20"
            >
